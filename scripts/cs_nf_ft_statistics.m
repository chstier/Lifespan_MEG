function [ stat_out ] = cs_nf_ft_statistics( all_subjects, opt, params )
%[ all_subjects ] = nf_ft_statistics( all_subjects, opt, params )
%   Will do generate a tabulated overview of metrics (all that are readable)
%   via nf_ft_load_metrics for any number of subject
% %s_
...
% CS Oct 2019 - order of tabulated overview was changed from the original script nf_ft_statistics.
% Easier to read out for global statistics
...
% load SUMA as ref
[~, ref_suma, ~]=nf_ft_load_metrics(all_subjects(1),opt.metrics{1},freq_n(1),params);
...
for st=1:length(opt.statistics)
 for m=1:length(opt.metrics)
  ...
  for n=1:length(all_subjects)
   ...
   [metric, suma, subjects_back]=nf_ft_load_metrics(all_subjects(n),opt.metrics{m},freq_n(f),params);
