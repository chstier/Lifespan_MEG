function [ stat_out ] = cs_create_global_files( all_subjects, opt, params )
% This script makes files with global statistics for selected metrics for
% frequency-specific statistical analyses
% 
% Written by NF and modified by Christina Stier, 2020/22
% Will do generate a tabulated overview of metrics (all that are readable)
% via nf_ft_load_metrics for any number of subjects
%
% all_subjects      = cell array of subjects (see nf_ft_all_subject)
%
...
% Main Loop - Per Metric and Per Frequency and per statistic requested
for st=1:length(opt.statistics)
 for m=1:length(opt.metrics)
  ...
  stat_out=cell((length(all_subjects)*length(opt.metrics)*length(opt.metrics))+1,stat_cols); 
  stat_out(1,1:length(stat_items))=stat_items; % print header
  if opt.report_regional
  stat_out(1,7:end)=use_rois_txt';
  end
  stat_row=2;
   ...
   if opt.report_regional && ~isequal(ref_suma{1}.annot_key,suma{1}.annot_key);
    ...
    for si=1:length(stat_items)
     switch stat_items{si}
      ...
      case 'Stat'
       stat_out{stat_row,si}=opt.statistics{st};
      case 'Global'
       stat_out{stat_row,si}=calc_stat(metric{1},opt.statistics{st});
     end
    end
    ...
    if opt.report_regional
     ...
     for r=1:length(use_rois_idx)
      ...
      if size(metric{1},1)==1 || size(metric{1},2)==1
       %1D
       stat_out{stat_row,si+r}=calc_stat(metric{1}(this_roi),opt.statistics{st}); 
      else
       %2D
       stat_out{stat_row,si+r}=calc_stat(metric{1}(this_roi,this_roi),opt.statistics{st}); 
      end
     end
    end
  ...
  % only save global values
  stat_value = stat_out(2:end,end);
  % save as csv
  nf_csvwrite(fullfile(opt.report_dir,['global_' opt.statistics{st} '_' opt.metrics{m} '_' freq_txt{f} '_descr' '.csv']),stat_out); % save full descr
  nf_csvwrite(fullfile(opt.report_dir,['global_' opt.statistics{st} '_' opt.metrics{m} '_' freq_txt{f} '_values' '.csv']),stat_value); % sa
