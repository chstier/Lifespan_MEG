function [ subject, data ] = cs_nmri_artifactrejection_reviewICA_grad(subject, data, params)
...
% load the components
if (~isfield(subject,'ICA_components_grad') || ~exist(subject.ICA_components_grad,'file'))
 error('ICA components not found, run nmri_artifactrejection_estimateICA first')
else
 % check if has selected
 content=who('-file',subject.ICA_components_grad);
 if ismember('selected',content)
  load(subject.ICA_components_grad,'comp','selected');
 else
  load(subject.ICA_components_grad,'comp');
 end
end

if (isfield(params,'useICA_clean') && params.useICA_clean==1)
 if (~isfield(subject,'cleanICA_grad_dataset'))
  subject.cleanICA_grad_dataset=fullfile(subject.analysis_dir,subject.id,'processed',['cleanICA_grad_' subject.id '_' subject.exam_id '.mat']);
 end
end
...
if (isfield(params,'useICA_clean') && params.useICA_clean==1)
 ...
 % save ICA data
 save(subject.ICA_components_grad,'comp','selected')
 ...
 subject=nmri_stamp_subject(subject,'artifactrejectionICA_grad',params);
  
 %% save ICA cleaned data - after ICA
 nmri_write_dataset(subject.cleanICA_grad_dataset,data,subject);
end 
...
if (isfield(params,'QC_plots') && params.QC_plots==1)
 ...
 if strcmpi(button,'Yes')
  ...
  cs_nmri_qcplot_power_grad(subject,data,params,['pow_grad_combined_plot_postICA_' datestr(now,'yyyymmdd')]);
