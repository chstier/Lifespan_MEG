function [ subject ] = nmri_qcplot_power_grad(subject, data, params, title_txt)
...
if (~exist('data','var') || isempty(data)) 
 if (isfield(subject,'cleanICA_grad_dataset') && exist(subject.cleanICA_grad_dataset,'file'))
  load(subject.cleanICA_grad_dataset,'data');
...
if (~exist('title_txt','var') || isempty(title_txt)) 
 title_txt='artifact_pow_grad_plot';
end
...
% 
% 
%   %system(['eog ' fullfile(subject.QCdir,['artifact_pow_grad_plot_' subject.id '_' subject.exam_id '.png'])])
% 
% end
  
  
  % try out combined planar
  
    cfgc = [];
    data2 = ft_combineplanar(cfgc, data);
   ...
   cfg.foi       = params.freqs(ff);
   cfg.tapsmofrq = params.tapsmofrq(ff); 
   pow_freq2(ff)       = ft_freqanalysis(cfg, data2);
   end
  ...
  subplot(rows,cols,i)
  title(params.freqsNames{i},'FontSize',12,'FontWeight','bold')
  ft_topoplotTFR(cfg,pow_freq2(i))
  end
  
  saveas(hFig,fullfile(subject.QCdir,[title_txt '_COMBINED_' subject.id '_' subject.exam_id '.png']),'png'); 
  % only show after save
  set(hFig,'Visible','on')
