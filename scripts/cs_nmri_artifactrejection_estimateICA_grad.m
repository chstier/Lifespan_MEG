function [ subject, data ] = cs_nmri_artifactrejection_estimateICA_grad(subject, data, params)
...
%  good_channels={};
%  for i=1:length(data.label_grad)
%   if ~any(strcmp(data.label_grad{i},data.bad_channels))
%    good_channels(end+1)=data.label_grad(i);
%   end
%  end
...
% take only gradiometers
 tcfg = [];
 tcfg.channel = {'MEG*2', 'MEG*3'};
 data_r=ft_selectdata(tcfg,data_r);
...
%  end
 ...
 if ~isreal(comp.topo)
  ...
  subject=nmri_stamp_subject(subject,'artifactrejectionICA_estimation_grad',params);
  % save ICA data
  disp('Writing ICA data...')
  save(subject.ICA_components_grad,'comp','subject')
 end
