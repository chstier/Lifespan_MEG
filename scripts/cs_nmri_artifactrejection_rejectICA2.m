...
mag_selected_manually = load(subject.ICA_components_mag, 'selected');
mag_man_comp = find(mag_selected_manually.selected == 0)';
...
grad_selected_manually = load(subject.ICA_components_grad, 'selected');
grad_man_comp = find(grad_selected_manually.selected == 0)';
...
%% Load selected components for GRAD and MAG based on correlation / coherence analysis and add components which might have been rejected manually
...
% now add manually rejected ICA-components (e.g. if physio-channel was missing)
% make sure not two have same components several times

comp_selected.mag = unique([comp_selected.mag mag_man_comp]); 
comp_selected.grad = unique([comp_selected.grad grad_man_comp]);
